...
const Manage = () => {
  ...
  const ReadBrandModels = async () => {
    let isMounted = true;
    const controller = new AbortController();
    try {
      setLoading(true);
      const response = await axiosPrivate.get('/BrandModel/GetAll', {
  ...
  const Update = (id) => {
    navigate('/Models/Update', { state: { id } });
  };
                      ...
                      <tr key={brand.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-sm font-medium text-gray-900">
                            <p>
                              {' '}
                              {brand.modelName} - {brand.modelNameAr} [
                              {brand.brand.name}]
                            </p>
                          </div>
                          {/* </div> */}
